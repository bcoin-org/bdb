...
class Level {
  ...
  open(options, callback) {
    ...
    req.onsuccess = (event) => {
      ...
      const tx = this.db.transaction([this.location], flags.READ_ONLY);
  ...
  put(key, value, callback) {
    ...
    const tx = this.db.transaction([this.location], flags.READ_WRITE);
...
class Batch {
  ...
  constructor(idb, options) {
    this.options = options || {};
    this.ops = [];
    this.db = idb.db;
    this.store = idb.store;
    this.location = idb.location;
    this.written = false;
  }
  ...
  write(callback) {
    ...
    for (const {type, key, value} of this.ops) {
      const store = tx.objectStore(this.location);
      switch (type) {
        case 'put': {
...
function isVersionError(event) {
  ...
  if (target.error)
    return target.error.name === 'VersionError';

  if (target.errorCode != null)
    return target.errorCode === 12;
...
function wrapError(event) {
  ...
  return new Error(`IndexedDB error: ${event.target.errorCode}.`);
}
