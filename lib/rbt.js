...
class RBT {
    ...
    while (x !== this.root && x.parent.color === RED) {
      if (x.parent === x.parent.parent.left) {
        ...
        if (!y.isNull() && y.color === RED) {
          ...
          x = /** @type {RBTNode}*/(x.parent.parent);
        } else {
          if (x === x.parent.right) {
            x = /** @type {RBTNode}*/(x.parent);
            this.rotl(x);
          }
          ...
          this.rotr(/** @type {RBTNode}*/(x.parent.parent));
        }
      } else {
        ...
        if (!y.isNull() && y.color === RED) {
          ...
          this.rotl(/** @type {RBTNode}*/(x.parent.parent));
        }
      }
    }
  ...
  removeNode(z) {
    ...
    if (y.color === BLACK)
      this.removeFixup(/** @type {RBTNode}*/(x));
  }
    ...
    while (x !== this.root && x.color === BLACK) {
      if (x === x.parent.left) {
        ...
        if (w.color === RED) {
          w.color = BLACK;
          x.parent.color = RED;
          this.rotl(/** @type {RBTNode}*/(x.parent));
          w = x.parent.right;
        }
        ...
        if (w.color === RED) {
          w.color = BLACK;
          x.parent.color = RED;
          this.rotr(/** @type {RBTNode} */(x.parent));
          w = x.parent.left;
        }
  ...
  successor(x) {
    if (!x.right.isNull()) {
      x = /** @type {RBTNode} */(x.right);

      while (!x.left.isNull())
        x = /** @type {RBTNode} */(x.left);
...
class RBTIterator {
  ...
  seekMin(key) {
    ...
    while (!root.isNull()) {
      ...
      if (cmp > 0) {
        current = root;
        root = /** @type {RBTNode} */(root.left);
      } else {
        root = /** @type {RBTNode} */(root.right);
      }
    }
...
/**
 * @param {RBTNode|RBTSentinel} parent
 * @param {RBTNode|RBTSentinel} node
 */

function copyLeft(parent, node) {
  if (!node.isNull()) {
    parent.left = /** @type {RBTNode} */(node).clone();
...
function copyRight(parent, node) {
  if (!node.isNull()) {
    parent.right = /** @type {RBTNode} */(node).clone();
